"""Connection lifecycle for Pleo Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from pleo_client import PleoClient
...
@chat.function(
    "connect_pleo",
    "Connect Pleo account via credentials.",
    action_type="write",
    chain_callable=True,
    event="pleo-connector.connect_pleo",
...
async def connect_pleo(ctx, params: ConnectParams) -> ActionResult[ConnectionRecord]:
    ...
    record = {
        "id": cid,
        "label": params.label or "Pleo Account",
    ...
    return ActionResult.success(ConnectionRecord(id=cid, label=record["label"], masked_key=_mask(params.api_key), base_url=params.base_url, is_active=True), summary="Pleo connected.")

@chat.function(
    "list_connections",
    "List connected Pleo accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_pleo",
    "Disconnect Pleo account.",
    action_type="write",
    chain_callable=True,
    event="pleo-connector.disconnect_pleo",
...
async def disconnect_pleo(ctx, params: ConnectionIdParams) -> ActionResult[DeleteResult]:
    ...
    return ActionResult.success(DeleteResult(id=target["id"], deleted=True, message="Disconnected successfully"), summary="Pleo disconnected.")
