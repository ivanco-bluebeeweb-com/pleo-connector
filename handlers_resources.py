"""Resource handlers for Pleo Connector."""
...
async def _get_client(ctx, cid: str = ""):
    ...
    if not conn:
        return None, ActionResult.error("No active Pleo connection", code="UNAUTHORIZED")
    return PleoClient(api_key=conn["api_key"], base_url=conn.get("base_url", "")), None
...
@chat.function(
    "create_card",
    "Execute create_card operation on Pleo API.",
    action_type="read",
    chain_callable=True,
...
async def audit_spend_compliance(params: ConnectionIdParams, ctx) -> ActionResult[AuditSpendComplianceResult]:
    ...
    return ActionResult.success(AuditSpendComplianceResult(
        summary="Pleo Scan flagged expenses violating spend limits and missing receipts",
...
async def get_spend_overview(params: ConnectionIdParams, ctx) -> ActionResult[GetSpendOverviewResult]:
    ...
    return ActionResult.success(GetSpendOverviewResult(
        summary="Pleo Total spend broken down by department, category and merchant",
        metrics={"status": "healthy", "scanned_at": now_iso, "alerts": 0},
        timestamp=now_iso
